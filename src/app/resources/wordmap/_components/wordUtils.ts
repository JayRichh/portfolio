...
export const STOP_WORDS = new Set([
  // Common words
  "and", "with", "using", "for", "in", "to", "of", "the", "a", "an",
  "on", "at", "by", "from", "up", "down", "into", "over", "after",
  // Technical terms
  "implementation", "development", "integration", "usage", "use",
  "basic", "simple", "first", "time", "built", "building", "created",
  "includes", "features", "working", "learning", "learned",
  "based", "various", "multiple", "different", "new", "custom",
  "setup", "support", "enabled", "making", "done", "doing",
  "project", "application", "app", "system", "component", "function",
  "data", "code", "coding", "programming", "software", "developer",
  // Additional technical terms
  "implementation", "interface", "module", "service", "class",
  "method", "object", "instance", "version", "update", "process",
  "server", "client", "user", "admin", "config", "configuration",
  "deploy", "deployment", "build", "release", "test", "testing",
  "debug", "debugging", "fix", "fixed", "implement", "implemented",
  "develop", "developed", "create", "created", "design", "designed",
  "architecture", "structure", "pattern", "practice", "solution"
]);
...
function areSimilar(word1: string, word2: string): boolean {
  ...
  const suffixes = ['ing', 'ed', 'er', 'est', 'ly', 'tion', 'ment', 'ness', 'able', 'ible'];
  for (const suffix of suffixes) {
    if (word1.endsWith(suffix) && word1.slice(0, -suffix.length) === word2) return true;
    if (word2.endsWith(suffix) && word2.slice(0, -suffix.length) === word1) return true;
  }
...
export function generateDatasetStats() {
  ...
  const totalFeatures = projectData.reduce((sum, p) => sum + p.details.features.length, 0);
  const totalChallenges = projectData.reduce((sum, p) => sum + p.details.challenges.length, 0);
  const totalLearnings = projectData.reduce((sum, p) => sum + p.details.learnings.length, 0);
  ...
  for (const [category, techs] of Object.entries(techCategories)) {
    if (techs.some(tech => tech.toLowerCase() === lowerWord)) {
      switch(category) {
        ...
        case "Libraries": return "framework";
        case "Tools":
        case "Databases":
        case "APIs":
        case "Platforms":
        case "Others": return "tool";
        default: return "concept";
      }
    }
  }

  if (/(api|service|platform|tool|cli|sdk|db|database)$/i.test(word)) return "tool";
...
export function processWords(samplingMethod: keyof typeof SAMPLING_METHODS = 'balanced') {
  ...
  projectData.forEach(project => {
    ...
    words.forEach(word => {
      ...
      if (existing) {
        ...
        existing.projectImpact = Math.max(existing.projectImpact, projectImpact * 0.8);
      } else {
        wordMap.set(word, {
          text: word,
  ...
  let words = Array.from(wordMap.values())
    .map(word => ({
      ...word,
      importance: calculateImportance(word.frequency, word.projectImpact, samplingMethod)
    }))
  ...
  words.forEach(word => {
    if (!filteredWords.some(existing => 
