...
export async function fetchGitHubContributions(): Promise<YearContributions[]> {
  ...
  try {
    ...
    const years =
      yearsResponse.data.data.user.contributionsCollection.contributionYears;
    ...
    for (const [index, year] of years.entries()) {
      ...
      const calendar =
        yearResponse.data.data.user.contributionsCollection
          .contributionCalendar;
      const weeks: ContributionWeek[] = calendar.weeks;
      const totalContributions = calendar.totalContributions;

      console.log(
        `Processing year ${year}, total contributions: ${totalContributions}`,
      );
